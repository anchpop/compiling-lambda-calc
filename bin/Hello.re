...
let pair = Lambda(Lambda(Lambda(App(Var(0), [Var(2), Var(1)]))));
let e1 = Lambda(App(Var(0), [Lambda(Lambda(Var(1)))]));
let e2 = Lambda(App(Var(0), [Lambda(Lambda(Var(0)))]));

let swap = Lambda(App(pair, [App(e2, [Var(0)]), App(e1, [Var(0)])]));
let lc = App(pair, [Z, S(Z)]);
